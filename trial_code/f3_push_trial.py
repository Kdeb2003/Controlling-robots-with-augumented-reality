...
def main():
    ...
    with context_manger:
        ...
        # start pose (side of object)
        start_pose = rcs.common.Pose(
            rpy_vector=np.array([-np.pi, 0.0, 0.0]),
            translation=np.array([0.5, 0.02, 0.2]),
        )
        ...
        # push along +Y
        end_pose = rcs.common.Pose(
            rpy_vector=np.array([-np.pi, 0.0, 0.0]),
            translation=np.array([0.5, 0.06, 0.2]),
        )
