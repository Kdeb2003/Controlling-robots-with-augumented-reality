...
Streams OBJECT-CENTER pose via send_fn.
"""
...
def _normalized_gripper_width(gripper, width_m: float) -> float:
    try:
        max_w = gripper.get_state().max_unnormalized_width
        if max_w is None:
...
def _ee_pose_stream(stop_event, robot, send_fn, ee_to_center_offset):
    ...
    while not stop_event.is_set():
        ...
        # Convert EE pose -> OBJECT CENTER pose
        center_pos = ee_pos + ee_to_center_offset

        T_center = T.copy()
        T_center[:3, 3] = center_pos

        unity_pose = robot_pose_4x4_to_unity(T_center)
...
def run_fr3_cart_grasp_xr(
    ...
    """
    Inputs are already in robot frame.
    ...
    with context:
        ...
        # Default: no offset
        ee_to_center_offset = np.zeros(3)
        width_norm = _normalized_gripper_width(gripper, object_width)

        cat = (object_cat or "").lower().strip()

        if cat in ("bowl", "cup", "ring"):
            ee_to_center_offset = np.array([0.0, object_width / 2.0, 0.0])
            target_pos = object_center - ee_to_center_offset
            width_norm = 0.1  # Just close a little bit for a "caging" grasp

        elif cat in ("pyramid", "star"):
            ee_to_center_offset = np.array([0.0, 0.0, -object_width / 2.0])
            target_pos = object_center - ee_to_center_offset
            width_norm = 0.1 # Just close a little bit for a "caging" grasp
            ...
            if width_norm >= 1.0:
                ...
                return
            target_pos = object_position

        # # Start EE stream (CENTERED) pose
        # stop_event = threading.Event()
        # stream_thread = threading.Thread(
        #     target=_ee_pose_stream,
        #     args=(stop_event, robot, send_fn, ee_to_center_offset),
        #     daemon=True,
        # )
        # stream_thread.start()
